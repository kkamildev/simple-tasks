...
const TaskModel : FC<Props> = ({data, number, onDelete}) => {


    return (
        <div className="p-4 shadow-md shadow-green-500/50 rounded-xl hover:scale-105 transition-transform duration-100 ease-in-out flex flex-col justify-between">
            <div>
                {
                ...
                <p className="font-bold text-zinc-700 mt-2 break-all">{truncate(data.content, 50)}{data.content.length > 50 && "..."}</p>
            </div>
            <div className="flex justify-between m-1 mt-4">
                <div className="flex gap-x-3">
                    <button title="Show more" className="btn bg-blue-600 hover:bg-blue-500 text-base! m-0!"><FontAwesomeIcon icon={faEye}/></button>
